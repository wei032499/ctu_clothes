...
$permission  = checkPermission(null, 'E02');
if ($permission < 1) {
...
<body>
    ...
    <section>
        <div class="container" style="margin: 20px auto">
            <div class="shadow bg-white " style="padding:0.15rem 1rem!important">
                ...
                <a href="/clothes/pattern/customManage.php" class="text-info">客製圖查詢</a>
            </div>
        </div>
    </section>
    <section>
        <div class="container" style="margin: 10px auto">
            <div class="shadow p-3 mb-5 bg-white rounded" style="padding:2.5rem!important">
                ...
                <div class="border " style="margin: 10px auto;padding:10px;overflow-x:hidden" id="searchBox">
                    ...
                    <div class="row" style="margin: auto;width: 100%;">
                        <div class="row col-sm" style="margin:10px auto">
                            <label for="" class="col-form-label" style="width: 100px;">客製圖編號：</label>
                            <div class="col-sm" style="min-width: 200px;">
                                <input type="text" class="form-control">
                            </div>
                        </div>
    ...
    <script>
        ...
        $(function() {
            ...
            $('#customTable').DataTable(opt_custom).draw();
            $.fn.dataTable.ext.search.push(
                function(settings, data, dataIndex) {
                    var Cust_Name = $('#searchBox input').eq(0).val();
                    var ProtoM_Name = $('#searchBox input').eq(1).val();
                    var Cust_Num = $('#searchBox input').eq(2).val();


                    if ((Cust_Name == '' || data[2].includes(Cust_Name)) &&
                        (ProtoM_Name == '' || data[5].includes(ProtoM_Name)) &&
                        (Cust_Num == '' || data[0].includes(Cust_Num))) {
                        return true;
                    }
                    return false;
            ...
            $("#customTable").wrap("<div style='overflow-x:auto'></div>");
        });

        function search() {
            $('#customTable').DataTable().draw();
        }
